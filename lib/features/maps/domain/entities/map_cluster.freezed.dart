// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'map_cluster.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
  'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#adding-getters-and-methods-to-our-models',
);

/// @nodoc
mixin _$MapCluster {
  LatLng get position => throw _privateConstructorUsedError;
  List<Campsite> get campsites => throw _privateConstructorUsedError;
  bool get isCluster => throw _privateConstructorUsedError;

  /// Create a copy of MapCluster
  /// with the given fields replaced by the non-null parameter values.
  @JsonKey(includeFromJson: false, includeToJson: false)
  $MapClusterCopyWith<MapCluster> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $MapClusterCopyWith<$Res> {
  factory $MapClusterCopyWith(
    MapCluster value,
    $Res Function(MapCluster) then,
  ) = _$MapClusterCopyWithImpl<$Res, MapCluster>;
  @useResult
  $Res call({LatLng position, List<Campsite> campsites, bool isCluster});
}

/// @nodoc
class _$MapClusterCopyWithImpl<$Res, $Val extends MapCluster>
    implements $MapClusterCopyWith<$Res> {
  _$MapClusterCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  /// Create a copy of MapCluster
  /// with the given fields replaced by the non-null parameter values.
  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? position = null,
    Object? campsites = null,
    Object? isCluster = null,
  }) {
    return _then(
      _value.copyWith(
            position:
                null == position
                    ? _value.position
                    : position // ignore: cast_nullable_to_non_nullable
                        as LatLng,
            campsites:
                null == campsites
                    ? _value.campsites
                    : campsites // ignore: cast_nullable_to_non_nullable
                        as List<Campsite>,
            isCluster:
                null == isCluster
                    ? _value.isCluster
                    : isCluster // ignore: cast_nullable_to_non_nullable
                        as bool,
          )
          as $Val,
    );
  }
}

/// @nodoc
abstract class _$$MapClusterImplCopyWith<$Res>
    implements $MapClusterCopyWith<$Res> {
  factory _$$MapClusterImplCopyWith(
    _$MapClusterImpl value,
    $Res Function(_$MapClusterImpl) then,
  ) = __$$MapClusterImplCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call({LatLng position, List<Campsite> campsites, bool isCluster});
}

/// @nodoc
class __$$MapClusterImplCopyWithImpl<$Res>
    extends _$MapClusterCopyWithImpl<$Res, _$MapClusterImpl>
    implements _$$MapClusterImplCopyWith<$Res> {
  __$$MapClusterImplCopyWithImpl(
    _$MapClusterImpl _value,
    $Res Function(_$MapClusterImpl) _then,
  ) : super(_value, _then);

  /// Create a copy of MapCluster
  /// with the given fields replaced by the non-null parameter values.
  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? position = null,
    Object? campsites = null,
    Object? isCluster = null,
  }) {
    return _then(
      _$MapClusterImpl(
        position:
            null == position
                ? _value.position
                : position // ignore: cast_nullable_to_non_nullable
                    as LatLng,
        campsites:
            null == campsites
                ? _value._campsites
                : campsites // ignore: cast_nullable_to_non_nullable
                    as List<Campsite>,
        isCluster:
            null == isCluster
                ? _value.isCluster
                : isCluster // ignore: cast_nullable_to_non_nullable
                    as bool,
      ),
    );
  }
}

/// @nodoc

class _$MapClusterImpl extends _MapCluster {
  const _$MapClusterImpl({
    required this.position,
    required final List<Campsite> campsites,
    required this.isCluster,
  }) : _campsites = campsites,
       super._();

  @override
  final LatLng position;
  final List<Campsite> _campsites;
  @override
  List<Campsite> get campsites {
    if (_campsites is EqualUnmodifiableListView) return _campsites;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_campsites);
  }

  @override
  final bool isCluster;

  @override
  String toString() {
    return 'MapCluster(position: $position, campsites: $campsites, isCluster: $isCluster)';
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$MapClusterImpl &&
            (identical(other.position, position) ||
                other.position == position) &&
            const DeepCollectionEquality().equals(
              other._campsites,
              _campsites,
            ) &&
            (identical(other.isCluster, isCluster) ||
                other.isCluster == isCluster));
  }

  @override
  int get hashCode => Object.hash(
    runtimeType,
    position,
    const DeepCollectionEquality().hash(_campsites),
    isCluster,
  );

  /// Create a copy of MapCluster
  /// with the given fields replaced by the non-null parameter values.
  @JsonKey(includeFromJson: false, includeToJson: false)
  @override
  @pragma('vm:prefer-inline')
  _$$MapClusterImplCopyWith<_$MapClusterImpl> get copyWith =>
      __$$MapClusterImplCopyWithImpl<_$MapClusterImpl>(this, _$identity);
}

abstract class _MapCluster extends MapCluster {
  const factory _MapCluster({
    required final LatLng position,
    required final List<Campsite> campsites,
    required final bool isCluster,
  }) = _$MapClusterImpl;
  const _MapCluster._() : super._();

  @override
  LatLng get position;
  @override
  List<Campsite> get campsites;
  @override
  bool get isCluster;

  /// Create a copy of MapCluster
  /// with the given fields replaced by the non-null parameter values.
  @override
  @JsonKey(includeFromJson: false, includeToJson: false)
  _$$MapClusterImplCopyWith<_$MapClusterImpl> get copyWith =>
      throw _privateConstructorUsedError;
}
